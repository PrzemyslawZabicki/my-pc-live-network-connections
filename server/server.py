            ...
            try:
                ...
                if c.pid:
                    try:
                        ...
                        proc_name = safe(proc.name)
                        exe       = safe(proc.exe)
                        cmdline   = safe(proc.cmdline)                  
                        ct = safe(proc.create_time)
                        # FIX: robust conversion: handle None, 0, floats/ints
                        try:
                            if isinstance(ct, (int, float)) and ct > 0:
                                create_time_iso = datetime.datetime.fromtimestamp(ct).isoformat()
                            else:
                        ...
                        backend_logger.exception(f"Access denied or process gone for pid {c.pid}")
